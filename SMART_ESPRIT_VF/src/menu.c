...
if(F!=NULL)
{
    ...
    fprintf(F,"%0.2s %s %s %s %s %d %s %s \n",A.id,A.type,A.plat,A.ing,A. date,A.nb,A.dessert,A.letype);
    fclose(F);
...
FW = fopen ("tr.txt", "w");
  ...
  if ((F!=NULL)&&(FW!=NULL))
 {
  ...
  while (fscanf (F, "%s %s %s %s %s %d %s %s",D.id,D.type,D.plat,D.ing,D. date,&D.nb,D.dessert,D.letype) == 8)
    if (strcmp (D.id, id) != 0) 
      fprintf (FW, "%s %s %s %s %s %d %s %s \n", D.id,D.type,D.plat,D.ing,D. date,D.nb,D.dessert,D.letype);
}
  fclose (F);
  fclose (FW);
remove("menu.txt");
...
F = fopen ("menu.txt", "r");
  ...
  if (F!=NULL)
{
  while (fscanf (F, "%s %s %s %s %s %d %s %s", Q.id,Q.type,Q.plat,Q.ing,Q. date, &Q.nb,Q.dessert,Q.letype) != EOF)
    if (strcmp (Q.id, id) == 0) 
        {
	...
	strcpy(A.dessert,Q.dessert);
	strcpy(A.letype,Q.letype);
...
FW = fopen ("tr.txt", "w");
  ...
  if ((F!=NULL)&&(FW!=NULL)) 
{
  while (fscanf (F, "%s %s  %s %s %s %d %s %s",c.id,c.type,c.plat,c.ing,c. date,&c.nb,c.dessert,c.letype) ==8) 
	
    if (strcmp (c.id, id) == 0)
     {

fprintf (FW, "%s %s %s %s %s %d %s %s\n",id,U.type,U.plat,U.ing,U.date,U.nb,U.dessert,U.letype);
     }
     else
     {

       fprintf (FW, "%s %s %s %s %s %d %s %s\n", c.id,c.type,c.plat,c.ing,c. date,c.nb,c.dessert,c.letype);
     }
}    
...
FILE *f;
	...
	char letype [30];
        ...
        f=fopen(l,"r");
while(fscanf(f,"%s %s %s %s %s %s %s %s\n",id,type,plat,ing,date,nb,dessert ,letype)!= EOF)
        {
         GtkTreeIter pIter;
