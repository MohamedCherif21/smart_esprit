...
void afficher(GtkWidget *liste,char txt[])
{
	...
	int j,m,a,nivo,g;
	char ch3[20],d_naissance[30],miveau[50];
...
while(fscanf(f,"%s %s %d %d %d %s %d %s %s %s %d %s \n",name,prename,&j,&m,&a,sexe,&nivo,ch3,adrisa,room,&g,h)!= EOF)
 {
	sprintf(miveau,"%d",j);
	strcpy(d_naissance,strcat(miveau,"-"));
	sprintf(miveau,"%d",m);
	strcat(d_naissance,strcat(miveau,"-"));
	sprintf(miveau,"%d",a);
	strcat(d_naissance,miveau);
	sprintf(miveau,"%d",nivo);
	strcat(miveau,ch3);

gtk_list_store_append(store,&iter);				gtk_list_store_set(store,&iter,NOM,name,PRENOM,prename,DATE,d_naissance,GENRE,sexe,NIVEAU,miveau,ADRESSE,adrisa,CHAMBRE,room,CIN,g,TEL,h,-1);
 }
...
while(fscanf(f,"%s %s %d %d %d %s %d %s %s %s %d %s \n",name,prename,&j,&m,&a,sexe,&nivo,ch3,adrisa,room,&g,h)!= EOF)
	{
			...
			gtk_list_store_set(store,&iter,NOM,name,PRENOM,prename,DATE,d_naissance,GENRE,sexe,NIVEAU,miveau,ADRESSE,adrisa,CHAMBRE,room,CIN,g,TEL,h,-1);
	}
	}
	else 
	{
while(fscanf(f,"%s %s %s %s %s %s %s %d %s\n",name,prename,d_naissance,sexe,miveau,adrisa,room,&g,h)!= EOF)
	{
		gtk_list_store_append(store,&iter);
		gtk_list_store_set(store,&iter,NOM,name,PRENOM,prename,DATE,d_naissance,GENRE,sexe,NIVEAU,miveau,ADRESSE,adrisa,CHAMBRE,room,CIN,g,TEL,h,-1);}	
	}
...
void rechercher(char tape[],int rechoix)
{
    FILE *f,*l;
    char name[30],d_naissance[20],miveau[15],h[11],adrisa[100],sexe[50];
   ...
   while(fscanf(f,"%s %s %d %d %d %s %d %s %s %s %d %s \n",name,prename,&j,&m,&a,sexe,&nivo,ch3,adrisa,room,&g,h)!= EOF)
		...
		strcat(miveau,ch3);
                ...
                fprintf(l,"%s %s %s %s %s %s %s %s %s\n",name,prename,d_naissance,sexe,miveau,adrisa,room,gcn,h);
            }
        }
	 if (trouve==0)
	    {
		...
		strcpy(miveau,"invalid");
		strcpy(adrisa,"invalid");
		strcpy(room,"invalid");
		g=0;
		strcpy(h,"invalid");
                fprintf(l,"%s %s %s %s %s %s %s %d %s\n",name,prename,d_naissance,sexe,miveau,adrisa,room,g,h);
	     }
      }
